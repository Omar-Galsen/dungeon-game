...
export default class DungeonScene extends Phaser.Scene {
  ...
  private readonly PORTAL_X = 2215;
  private readonly PORTAL_Y = 360;
  private readonly PORTAL_DISTANCE = 150;
  ...
  preload() {
    ...
    for (let i = 1; i <= 8; i++) {
      for (const d of ["down", "up", "left", "right"]) {
        this.load.image(`d2_sword_idle_${d}_${i}`, `./assets/sprites/sword/idle/sword_idle_${d}_${i}.png`);
        this.load.image(`d2_sword_walk_${d}_${i}`, `./assets/sprites/sword/walk/sword_walk_${d}_${i}.png`);
        this.load.image(`d2_sword_attack_${d}_${i}`, `./assets/sprites/sword/attack/sword_attack_${d}_${i}.png`);
      }
    }
  ...
  create() {
    ...
    for (const box of collisionData?.boxes ?? []) {
      ...
      wall.setVisible(false).setDisplaySize(box.width, box.height);
      wall.refreshBody();
    }

    this.createAnimations();
    this.createAcidTexture();
    this.createVillagePortal();
    ...
    this.add.text(this.ENTRANCE_X - 20, this.ENTRANCE_Y + 90, "X TO RETURN", {
    ...
    this.objectiveText = this.add.text(18, 62, "DEFEAT ALL GREEN SLIMES TO UNLOCK THE PORTAL", {
      fontFamily: "Arial", fontSize: "16px", fontStyle: "bold", color: "#d8b4fe", stroke: "#000000", strokeThickness: 4,
      backgroundColor: "#20102d", padding: { x: 10, y: 5 },
    }).setScrollFactor(0).setDepth(1000);
  ...
  update(time: number, delta: number) {
    ...
    if (!this.swordAttackPlaying) {
      ...
      this.player.play(dx !== 0 || dy !== 0 ? `d2_sword_walk_${this.currentDirection}` : `d2_sword_idle_${this.currentDirection}`, true);
    ...
    if (Phaser.Input.Keyboard.JustDown(this.exitKey)) {
      if (this.tryEnterVillagePortal()) return;
      if (Phaser.Math.Distance.Between(this.player.x, this.player.y, this.ENTRANCE_X, this.ENTRANCE_Y) <= this.EXIT_DISTANCE) {
  ...
  private swordAttack() {
    ...
    if (target && target.distance <= this.SWORD_RANGE) {
      ...
      this.updateSlimeHearts(target.slime);
      ...
      if (target.slime.hp <= 0) {
        ...
        target.slime.hearts.forEach((heart) => heart.destroy());
        target.slime.hearts = [];
        this.updatePortalState();
      }
    }
  ...
  private createVillagePortal() {
    const glow = this.add.circle(0, 0, 58, 0x8b5cf6, 0.16);
    const ring = this.add.circle(0, 0, 42, 0x4c1d95, 0.55).setStrokeStyle(5, 0xc084fc, 0.95);
    const core = this.add.circle(0, 0, 27, 0xa855f7, 0.75).setStrokeStyle(4, 0xf0abfc, 1);
    const swirl = this.add.text(0, -2, "✦", { fontSize: "42px", color: "#f5d0fe", stroke: "#581c87", strokeThickness: 5 }).setOrigin(0.5);
    this.portal = this.add.container(this.PORTAL_X, this.PORTAL_Y, [glow, ring, core, swirl]).setDepth(12).setVisible(false);
    this.tweens.add({ targets: [glow, ring], scaleX: 1.18, scaleY: 1.18, alpha: 0.55, duration: 800, yoyo: true, repeat: -1, ease: "Sine.easeInOut" });
    this.tweens.add({ targets: core, angle: 360, duration: 2200, repeat: -1 });
    this.portalHint = this.add.text(this.PORTAL_X, this.PORTAL_Y + 88, "", {
      fontFamily: "Arial", fontSize: "18px", fontStyle: "bold", color: "#f5d0fe", stroke: "#000000", strokeThickness: 5,
      backgroundColor: "#24102f", padding: { x: 9, y: 5 },
    }).setOrigin(0.5).setDepth(50).setVisible(false);
  }

  private updatePortalState() {
    const remaining = this.acidSlimes.filter((slime) => slime.sprite.active && slime.hp > 0).length;
    if (remaining > 0) {
      if (this.objectiveText) this.objectiveText.setText(`DEFEAT GREEN SLIMES: ${remaining} REMAINING`);
      return;
    }
    if (!this.portalUnlocked) {
      this.portalUnlocked = true;
      this.registry.set("dungeon2Cleared", true);
      this.portal.setVisible(true);
      this.portalHint.setVisible(true).setText("PORTAL OPEN — PRESS X");
      if (this.objectiveText) this.objectiveText.setText("PORTAL UNLOCKED — GO TO RIVERSIDE VILLAGE");
      this.cameras.main.flash(400, 140, 60, 255);
    }
    const near = Phaser.Math.Distance.Between(this.player.x, this.player.y, this.PORTAL_X, this.PORTAL_Y) <= this.PORTAL_DISTANCE;
    this.portalHint.setText(near ? "X — ENTER RIVERSIDE VILLAGE" : "PORTAL TO RIVERSIDE VILLAGE");
  }

  private tryEnterVillagePortal() {
    if (!this.portalUnlocked) return false;
    const near = Phaser.Math.Distance.Between(this.player.x, this.player.y, this.PORTAL_X, this.PORTAL_Y) <= this.PORTAL_DISTANCE;
    if (!near) return false;

    this.player.setVelocity(0, 0);
    this.registry.set("playerHealth", this.playerHealth);
    this.portalHint.setText("A MYSTERIOUS FORCE SURROUNDS YOU...");
    this.cameras.main.fadeOut(700, 125, 55, 190);
    this.time.delayedCall(760, () => this.scene.start("VillageScene"));
    return true;
  }
  ...
  private spitAcid(slime: Phaser.Physics.Arcade.Sprite) {
    ...
    acid.setDisplaySize(38, 28).setDepth(25).setBlendMode(Phaser.BlendModes.NORMAL);
    const angle = Phaser.Math.Angle.Between(slime.x, slime.y, this.player.x, this.player.y);
    acid.setRotation(angle);
    this.physics.velocityFromRotation(angle, this.ACID_SPEED, acid.body.velocity);
    this.tweens.add({ targets: acid, scaleX: { from: 0.78, to: 1.18 }, scaleY: { from: 1.18, to: 0.72 }, angle: { from: -7, to: 7 }, duration: 110, yoyo: true, repeat: -1, ease: "Sine.easeInOut" });
  }

  private createAcidTexture() {
    if (this.textures.exists("d2_acid_blob")) return;
    const graphics = this.make.graphics({ x: 0, y: 0 }, false);
    graphics.fillStyle(0x4fd329, 1); graphics.fillEllipse(20, 16, 30, 20); graphics.fillCircle(9, 17, 7); graphics.fillCircle(29, 12, 7); graphics.fillCircle(31, 21, 5);
    graphics.fillStyle(0x258d22, 0.95); graphics.fillCircle(13, 23, 5); graphics.fillCircle(25, 25, 4); graphics.fillEllipse(34, 18, 7, 11);
    graphics.fillStyle(0xc8ff8a, 0.95); graphics.fillCircle(14, 11, 4); graphics.fillCircle(23, 9, 3); graphics.fillStyle(0xffffff, 0.65); graphics.fillCircle(12, 9, 2);
    graphics.generateTexture("d2_acid_blob", 40, 32); graphics.destroy();
  }
